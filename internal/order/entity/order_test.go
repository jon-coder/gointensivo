...
func TestGivenAValidParams_WhenICallNewOrder_ThenIReceiveCreateOrderWithAllParams(t *testing.T) {
	order := Order{ID: "123", Price: 12.0, Tax: 2.0}
...
func TestGivenAValidParams_WhenICallNewOrderFunc_ThenIReceiveCreateOrderWithAllParams(t *testing.T) {
	order, err := NewOrder("123", 12.0, 2.0)
	assert.Nil(t, err)
	assert.Equal(t, "123", order.ID)
	assert.Equal(t, 12.0, order.Price)
	assert.Equal(t, 2.0, order.Tax)
...
func TestGivenAPriceAndTax_WhenICallCalculatePrice_ThenIShouldSetFinalPrice(t *testing.T) {
	...
	assert.Equal(t, 14.0, order.FinalPrice)
}
